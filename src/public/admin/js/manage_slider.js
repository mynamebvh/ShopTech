...
const loadUsers = () => {
  const api = '/api/v1/sliders?sortBy=createdAt:desc';
  var table = $('#file-datatable').DataTable({
    ...
    columns: [
      ...
      },
      {
        data: 'isShow',
        render: function (data, type, row) {
          return data == true ? '<span class="text-info tx-bold">Hiển thị</span>' : '<span class="text-info tx-bold">Ẩn</span>';
        },
      },
...
const deleteSlider = () => {
  ...
  $('#file-datatable tbody').on('click', 'button#btnDelete', function () {
   slider = $('#file-datatable').DataTable().row($(this).parents('tr')).data();
  });

  $('#lock').on('click', async function () {
    const response = await fetch(`/api/v1/sliders/${slider.id}`, {
      method: 'DELETE',
    });
