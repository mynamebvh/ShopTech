...
const loadUsers = () => {
  const api = '/api/v1/products?sortBy=createdAt:desc';
    ...
    ajax: {
      ...
      dataSrc: function (json) {
        json.data.forEach((element) => {
          element.method = `
          <a href="/admin/manage-product/edit/${element.id}">
...
const deleteProduct = () => {
  $('#file-datatable tbody').on('click', 'button#btnDelete', function () {
    const product = $('#file-datatable').DataTable().row($(this).parents('tr')).data();
